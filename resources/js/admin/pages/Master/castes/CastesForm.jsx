...
const CastesForm = (props) => {
    const { auth, caste } = props;
    const [name, setName] = useState(caste ? caste.name : '');
    const [nameNp, setNameNp] = useState(caste ? caste.name_np : '');
    // const [status, setStatus] = useState(castes ? castes.status : '');
    // const [category, setCategory] = useState(castes ? castes.category : '');
    // const [subcategory, setSubcategory] = useState(castes ? castes.subcategory : '');
    ...
    const handleSubmit = (event) => {
        ...
        formData.append('_method', caste ? 'PUT' : 'POST');
        formData.append('name', name);
        formData.append('name_np', nameNp);

        const routeName = caste ? 'castes.update' : 'castes.store';
        const routeParams = caste ? { id: caste.id } : {};
        router.post(route(routeName, routeParams), formData, {
            forceFormData: true,
            preserveState: true,
            preserveScroll: true,
            onSuccess: () => toastr.success(caste ? 'Caste updated successfully' : 'Caste created successfully'),
        });
    };
