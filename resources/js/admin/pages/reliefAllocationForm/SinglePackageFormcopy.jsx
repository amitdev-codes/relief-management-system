...
const SinglePackageForm = ({ processedAllocations, relief_types, reliefFundAllocation = {}, users, processing }) => {
    // console.log(reliefFundAllocation)
    ...
    const [relief_sub_category_id, setReliefSubCategoryId] = useState(reliefFundAllocation ? reliefFundAllocation.relief_sub_category_id : '');
    ...
    const handleGridChange = async (index, field, value) => {
        ...
        if (field === 'relief_type_id') {
            ...
            const selectedSubCategory = (mst_relief_sub_categories[reliefEntries[index].relief_type_id] || []).find(subCat => subCat.id === value);
    ...
    const fetchSubCategory = async () => {
        if (relief_type_id) {
            try {
                const response = await axios.get(`/admin/api/fetch_relief_subcategory/${relief_type_id}`);
                const updatedEntries = [...reliefEntries];
                updatedEntries[index].subCategories = response.data;
                setMstReliefSubCategory(response.data);
            } catch (error) {
    ...
    useEffect(() => {
        ...
        reliefEntries.forEach(async (entry, index) => {
        ...
        if (reliefFundAllocation.file_uploads) {
            setFilePreview(reliefFundAllocation.file_uploads);
        }
    }, [reliefEntries, mst_relief_sub_categories, reliefFundAllocation.file_uploads]);
    ...
    const handleSubmit = (e) => {
        ...
        formData.append('applicant_ids', JSON.stringify(applicant_ids.split(',').map(id => id.trim()))); // Convert to JSON array
        formData.append('incident_description', incident_description);
        formData.append('remarks', remarks);
        formData.append('single_package', JSON.stringify(reliefEntries));
        if (file_uploads instanceof File) {
            formData.append('file_uploads', file_uploads);
        }

        // Convert relief_entries to the required format
        ...
        const url = isEditMode
            ? route('reliefFundAllocation.update', reliefFundAllocation.id)
            : route('reliefFundAllocation.store');
                            ...
                            <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                                ...
                                >
                                    {filePreview ? (
                                        <img
                                            // src={filePreview}
                                            src={filePreview instanceof File ? URL.createObjectURL(filePreview) : `/storage/${filePreview}` || 'https://as2.ftcdn.net/v2/jpg/07/86/72/89/1000_F_786728988_QyuP5WkUfZMlGMEMltILI72HWVtkEyYx.jpg'}
                                            alt="File Preview"
                                            id="file_uploads"
